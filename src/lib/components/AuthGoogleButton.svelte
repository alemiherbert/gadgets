<script lang="ts">
	type AuthContext = 'signin' | 'signup';

	let {
		context = 'signin',
		href = '/auth/google'
	}: { context?: AuthContext; href?: string } = $props();

	const dividerText = $derived(context === 'signup' ? 'Or sign up with' : 'Or continue with');
	const buttonText = $derived(context === 'signup' ? 'Continue with Google' : 'Sign in with Google');
</script>
